...
画像素材は`public/images/`に配置しています。日時はアクセス時に閲覧端末の時刻を取得し、固定表示します。参照元は`document.referrer`のホスト名を表示し、取得できない場合は`None`を表示します。ID・経歴・Failed表示はFigmaの静的な表示内容をそのまま使用しています。狭い画面では余白と文字サイズを調整しています。
